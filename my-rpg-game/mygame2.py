...
def game_dialog(dialog: str):
    ...
    if dialog == 'keyboard_menu':
        ...
        #print(my_main_character)
        print(dash * 20)
    elif dialog == 'defeated_enemy':
        print(
        '''
        You have defeated the enemy! A surge of power runs through your veins... 
...
def fight(player: dict, enemy_type: dict):
    ...
    print(f'Enemy Stats: {enemy_type}')
    # Roll dice
    ...
    if enemy_type["HP"] <= 0:
        game_dialog('defeated_enemy')
        strength_increase = random.randint(1,10)
        print(f'... you have gained an additional +{strength_increase} Strength!')
        #game_dialog('player_stats')
        # Resetting Enemy Health
        if enemy_type["Name"] == 'Goblin':
            enemy_type["HP"] = 20 
        elif enemy_type["Name"] == 'Orc':
            enemy_type["HP"] = 60
        elif enemy_type["Name"] == 'Warrior':
            enemy_type["HP"] = 70
        elif enemy_type["Name"] == 'Orc':
            enemy_type["HP"] = 200
